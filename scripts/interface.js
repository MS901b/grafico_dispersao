...
document.observe("dom:loaded", function() {
    ...
    if ($('associado')) $('associado').hide();
    if ($('associacao')) $('associacao').hide();
...
var Corrigir = Class.create({
    ...
    seleciona: function() {
        if (!NoValorInicial && !this.selecionada) {
            ...
            if (this.li.viewportOffset().top + this.li.getHeight() + 60 > document.viewport.getHeight() && !this.scrolando) {
...
var MsgErro = Class.create({
    ...
    abre: function() {
        if (!this.aberto) {
            this.aberto = true;
            new Effect.BlindDown(this.divCont, { duration: 0.3, beforeStart: this.tiraHover.bind(this), afterFinish: this.poemHover.bind(this) });
        }
    },
    fecha: function(ev) {
        if (ev) ev.stop();
        if (this.aberto) {
            this.aberto = false;
            this.a.up('.questao_item').down('a.feedback').focus();
            new Effect.BlindUp(this.divCont, { duration: 0.3, beforeStart: this.tiraHover.bind(this), afterFinish: this.poemHover.bind(this) });
        }
    },
...
function removeEsperando(Item, msg) {
    var QuestaoEsperando = PegaQuestao(Item);
    var filho = QuestaoEsperando.divCont.down('.msg_valor_inicial');
    QuestaoEsperando.travado = false;

    if (filho) {
        if (msg != null) {
            filho.down('p').hide();
            filho.down('p.depois').show().update(msg);
        } else {
            filho.hide();
        }
    }
