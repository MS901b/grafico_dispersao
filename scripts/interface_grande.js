    ...
    QuestoesDados.each(function(dados) {
        ...
        while (Contador < QuestaoDados.length) {
            ...
            if (ItemDados.get('tipo') != 'valor_inicial') {
                ...
                divCont.insert(new Element('div', { className: 'limpador' }));
                Item.insert(divCont);
            ...
            Item.insert(new Element('div', { className: 'canto sup_esq' }));
            Item.insert(new Element('div', { className: 'canto sup_dir' }));
            Item.insert(new Element('div', { className: 'canto inf_esq' }));
            Item.insert(new Element('div', { className: 'canto inf_dir' }));
...
var ValorInicial = Class.create({
    ...
    initialize: function(pai, divCont) {
        ...
        if (this.pai)
            this.a.observe('click', function(ev) {
                ev.stop();
                this.seleciona();
                this.pai.scrollTo()
            }.bind(this));
    ...
    initialize: function(selecionou, corrigir, ids, msg, divCont, tipo, travado) {
        ...
        ids.each(function(item) {
            ...
            Event.observe(it, 'keypress', function(ev) {
                ...
                if (key != Event.KEY_TAB && !ev.ctrlKey)
                    this.nada();
            }.bind(this));
        ...
        this.divCont.insert(this.calcoPre = new Element('div', { rel: 'pre_calco', className: 'limpador', style: 'height: 0; width: 100%;' }));
        this.divCont.insert(this.a);
        this.divCont.insert(this.calcoPos = new Element('div', { rel: 'pos_calco', className: 'limpador', style: 'height: 0; width: 100%;' }));
...
var MsgErro = Class.create({
    ...
    abre: function() {
        if (!this.aberto) {
            this.aberto = true;
            new Effect.BlindDown(this.divCont, { duration: 0.3, beforeStart: this.tiraHover.bind(this), afterUpdate: this.ajusta, afterFinish: this.poemHover.bind(this) });
        }
    },
    fecha: function(ev) {
        if (ev) ev.stop();
        ...
        if (this.aberto) {
            new Effect.BlindUp(this.divCont, { duration: 0.3, beforeStart: this.tiraHover.bind(this), afterUpdate: this.ajusta, afterFinish: this.poemHover.bind(this) });
            this.aberto = false;
        }
    },
    tiraHover: function(eff) {
        if (Prototype.Browser.IE) {
            var item = PosicaoAtual.Item;
            while (PosicaoAtual.Item++ && PegaQuestao(PosicaoAtual)) {
                PegaQuestao(PosicaoAtual).divCont.up().removeClassName('questao_item');
                PegaQuestao(PosicaoAtual).divCont.up().addClassName('questao_item_hack');
            }
            PosicaoAtual.Item = item;
        }
    },
    poemHover: function(eff) {
        if (Prototype.Browser.IE) {
            var item = PosicaoAtual.Item;
            while (PosicaoAtual.Item++ && PegaQuestao(PosicaoAtual)) {
                PegaQuestao(PosicaoAtual).divCont.up().addClassName('questao_item');
                PegaQuestao(PosicaoAtual).divCont.up().removeClassName('questao_item_hack');
            }
            PosicaoAtual.Item = item;
            PegaQuestao(PosicaoAtual).divCont.setStyle({ padding: '0' });
        }
        this.ajusta();
    }
});
...
function PegaQuestao(Posicao) {
    ...
    if ($H(Posicao).get('Questao'))
        if (questao = $H(Questoes[Posicao.Parte]).get(Posicao.Questao))
            if (questao.itens && questao.itens.length >= Posicao.Item + 1)
                if (questao.itens[Posicao.Item].Corrigir)
                    return questao.itens[Posicao.Item].Corrigir;
    return null;
}
    ...
    if (deu_erro == true) {
        if (corrigindo == false) {
            CorrigeTudoPopup = new PopupCallback(
                $('corrigir_tudo'),
                'Algumas questões estão incorretas.', ['seta_baixo', 'esquerda'], 12,
                function() {
                    if (!PegaQuestao(incorreta).travado)
